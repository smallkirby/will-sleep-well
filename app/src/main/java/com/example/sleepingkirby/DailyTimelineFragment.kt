...
class DailyTimelineFragment : Fragment() {
    ...
    private fun onLogSelected(task: DailyDefinition) {
        ...
        TweetLogDialogFragment(task).show(parentFragmentManager, "dotweet")
    }
    ...
    class TweetLogDialogFragment(private val task: DailyDefinition): DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            return AlertDialog.Builder(requireContext())
                    .setTitle("Do you want to tweet?")
                    .setPositiveButton("Tweet") { dialogInterface: DialogInterface, i: Int ->
                        doTweet(task)
                    }
        ...
        private fun doTweet(task: DailyDefinition) {
            var text = task.name
            text += "\n[auto-generated by skbsleeping]"
            val browserIntent = Intent(Intent.ACTION_VIEW, Uri.parse("https://twitter.com/intent/tweet?text=${text}"))
